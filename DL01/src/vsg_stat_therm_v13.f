...
!----------------- specify whether single zone or multizone ------------------
      ...
      IF(GEOM.EQ.'slab')GEOM='SLAB'
      IF(GEOM.EQ.'lirg')GEOM='LIRG'

      IF(GEOM.EQ.'ISRF')THEN
         WRITE(0,FMT='(A,A)')GEOM,' = GEOM'
         NZONE=1
      ELSEIF(GEOM.EQ.'SLAB')THEN
         WRITE(0,FMT='(A,A)')GEOM,' = GEOM'
         READ(3,*)NZONE
         READ(3,*)AVMAX
         WRITE(0,FMT='(I5,A)')NZONE,' = number of zones'
         WRITE(0,FMT='(F10.3,A)')AVMAX,' = A_V (mag) thickness of slab'
      ELSEIF(GEOM.EQ.'LIRG')THEN
         WRITE(0,FMT='(A,A)')'GEOM=',GEOM,
     &             ' mix of stars and dust'
         NZONE=1
      ELSE
      ...
      IF(GEOM.EQ.'SLAB'.OR.GEOM.EQ.'LIRG')THEN

! need to obtain dust attenuation cross section per H
! AWAV(I)=A_lambda/A_V
! scattering is neglected

         CALL EXT_HD20(0.55,TAUVNH)
         OPEN(UNIT=14,FILE='extcurv.out')
         WRITE(14,FMT='(A)')'  wave(um)  A_lambda/A_V'
         DO I=1,NISRF

! ISRF_WL is in cm

            WAVEUM=ISRF_WL(I)/1.E-4
            CALL EXT_HD20(WAVEUM,TAUNH)
            AWAV(I)=TAUNH/TAUVNH
            WRITE(14,FMT='(1PE9.3,E10.3)')ISRF_WL(I),AWAV(I)
         ENDDO
         CLOSE(14)

!*** diagnostic
!         write(0,fmt='(A,1pe10.3)')
!     &      'vsg_stat_therm_v12 ckpt 0.5: isrf_wl(1)=',isrf_wl(1)
!         write(0,fmt='(a,1pe10.3)')
!     &      '                               awav(1)=',awav(1)
!***
      ENDIF   ! endif geom= pdr or slab or lirg

!--------------------- specify the radiation field ---------------------

!     radiation='MMPISRF' for MMP ISRF

!               'BLACKBD' for dilute blackbody
!                         TSTAR = stellar temperature (K)
!                         LUMINOSITY = stellar luminosity/L_sol
!                         LOCATION = distance from star (cm)

!               'M31BLGE' for spectrum of M31 bulge stars 

!               'STRBRST' for Z=0.2 instant. starburst
!                         AGE = time since burst (yr)
!                         RADFLD = dF/dlambda (erg s-1 cm-3)
!                                  at 1 kpc distance for 1e6 Msol burst

!               'BC_2003' for Bruzual-Charlot (2003) models with
!                         AGE = time since burst (yr)
!                             = 3e6, 1e7, 1e8, 3e8, 1e9
!                         ZBC = metallicity
!                             = 0.02 or 0.0004

!               'BPASS' for BPASS models with
!                         AGE = time since burst (yr)
!                             = 3e6, 1e7, 1e8, 3e8, 1e9
!                         ZBPASS = metallicity
!                             = 0.02 or 0.001

!               'KURUCZ'  for selected Kurucz model atmospheres
!                         TEFF = effective temperature

!               'SSPEXPT' single stellar population experiment
!
!*** diagnostic
      write(0,fmt='(a)')
...
! UMMP = u(starlight)/u(mMMP83 starlight)
         ...
         READ(3,*)UMMP
      ELSEIF(RADIATION.EQ.'BLACKBD')THEN
         READ(3,*)TSTAR
         READ(3,*)LUMINOSITY
         READ(3,*)LOCATION

! for blackbody, UMMP specifies ratio of energy densities
!                               (not heating rate)
!     ** beware of inconsistency with usage of U for other cases **
!        mMMP83 starlight : u = 1.043e-12 erg cm-3

         UMMP=(3.826E33*LUMINOSITY/(4.*PI*LOCATION**2))/
     &        (C*1.043E-12)

! read in luminosity in solar luminosities
!     and LOCATION = distance in cm
! calculate stellar luminosity and radius in cgs units

         LUMINOSITY=LUMINOSITY*3.826D33
         RADIUS_STAR=(LUMINOSITY/(4.*3.1416*5.67D-5*TSTAR**4.))**0.5

! OMEGA = dilution factor = pi(r*/D)^2/4*pi = (r/2D)**2

         OMEGA=(RADIUS_STAR/(2.*LOCATION))**2.

      ELSEIF(RADIATION.EQ.'M31BLGE')THEN

         READ(3,*)UMMP

      ELSEIF(RADIATION.EQ.'STRBRST')THEN
         READ(3,*)AGE   ! age in years
         READ(3,*)UMMP  ! (0.1um dust heating rate)/(mMMP83)

      ELSEIF(RADIATION.EQ.'BC_2003')THEN
         READ(3,*)AGE           ! age in years
         READ(3,*)ZBC           ! metal mass fraction (0.0004 or 0.02)
         READ(3,*)UMMP          ! (0.1um dust heating rate)/(mMMP83)

      ELSEIF(RADIATION.EQ.'KURUCZ')THEN
         READ(3,*)TEFF  ! effective temperature (K)
         READ(3,*)UMMP  ! (0.1um dust heating rate)/(mMMP83)

      ELSEIF(RADIATION.EQ.'SSPEXPT')THEN
         READ(3,*)AGE   ! age in years
         READ(3,*)UMMP

      ELSEIF(RADIATION.EQ.'BPASS')THEN
         READ(3,*)AGE           ! age in years
         READ(3,*)ZBPASS        ! metal mass fraction (0.02 or 0.001)
         READ(3,*)UMMP          ! (0.1um dust heating rate)/(mMMP83)
...
! Calculate some ISRF parameters and write them out
      ...
      IF(RADIATION.EQ.'MMPISRF')THEN
         ...
         WRITE(CDESCR(2),FMT='()')
      ELSEIF(RADIATION.EQ.'BLACKBD')THEN
         WRITE(CDESCR(1),FMT='(A,1PE10.3,A,1PE10.3,A,A,1PE10.3,A)')
     &      'Dilute blackbody RF, T_bb=',TSTAR,
     &      ' L=',LUMINOSITY,' erg s-1',' d=',LOCATION,' cm'
         WRITE(CDESCR(2),FMT='()')
      ELSEIF(RADIATION.EQ.'M31BLGE')THEN
         WRITE(CDESCR(1),FMT='(A)')' M31 bulge star spectrum'
         WRITE(CDESCR(2),FMT='()')
      ELSEIF(RADIATION.EQ.'STRBRST')THEN
         WRITE(CDESCR(1),FMT='(A)')' '
         WRITE(CDESCR(2),FMT='(A,1PE10.3,A,1PE10.3)')
     &        ' starburst, age (yr) =',AGE,' U=',UMMP
      ELSEIF(RADIATION.EQ.'BC_2003')THEN
         WRITE(CDESCR(1),FMT='(A)')' '
!*** diagnostic
         write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.1'
!***
         WRITE(CDESCR(2),FMT='(A,1PE10.3,A,1PE10.3,A,1PE10.3)')
     &        ' Bruzual+Charlot 03 strbrst, age (yr) =',AGE,
     &        ' Z=',ZBC,' U=',UMMP
!*** diagnostic
         write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.2'
!***
      ELSEIF(RADIATION.EQ.'BPASS')THEN
         WRITE(CDESCR(1),FMT='(A)')' '
!*** diagnostic
         write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.21'
!***
         WRITE(CDESCR(2),FMT='(A,1PE10.3,A,1PE10.3,A,1PE10.3)')
     &        ' BPASS strbrst, age (yr) =',AGE,
     &        ' Z=',ZBPASS,' U=',UMMP
!*** diagnostic
         write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.22'
!***
      ELSEIF(RADIATION.EQ.'KURUCZ')THEN
         WRITE(CDESCR(1),FMT='(A)')' '
!*** diagnostic
         write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.23'
!***
         WRITE(CDESCR(2),FMT='(A,1PE10.3,A,1PE10.3,A,1PE10.3)')
     &        ' Kurucz model atmosphere, Teff =',TEFF,
     &        ' (K)  U=',UMMP
!*** diagnostic
         write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.24'
...
!***
     ...
     &         ' MMP83 spec, log(U)=',LOG10(UMMP)
         ELSEIF(RADIATION.EQ.'BLACKBD')THEN
            CALL BB_RADIATION_FIELD(TSTAR,OMEGA,ISRF_WL(I),ISRF(I))
            WRITE(RADDESCRIPT,FMT='(A5,1PE10.2,A10,1PE10.3)')
     &         ' Tbb=',TSTAR,' dil.fac.=',OMEGA/(4.*PI)
         ELSEIF(RADIATION.EQ.'M31BLGE')THEN
            CALL M31BULGE_FIELD(ISRF_WL(I),ISRF(I))

! multiply such that heating rate is UMMP * heating rate for MMP83 ISRF
! M31BULGE_FIELD returns ISRF at R=1kpc from pt src with bulge luminosity
! bulge field at 1 kpc corresponds to U=5.255 times MMP83 heating rate
!            ISRF(I)=(UMMP/5.255)*ISRF(I)

! 2020.09.21 (BTD) correction to mMMP for b/a=0.5 P=0.2 astrodust
!            ISRF(I)=(UMMP/5.034)*ISRF(I)

! 2021.02.01 (BTD) correction to mMMP for b/a=1.6 P=0.2 astrodust
            ISRF(I)=(UMMP/5.049)*ISRF(I)


            WRITE(RADDESCRIPT,FMT='(A6,F7.3,A17)')
     &         ' logU=',LOG10(UMMP),' M31 bulge spectrum'
         ELSEIF(RADIATION.EQ.'STRBRST')THEN
!*** diagnostic
            if(i.eq.1)write(0,fmt='(a,1pe10.3,a,e10.3)')
     &         'vsg_stat_therm_v12 ckpt 9.51, age=',age,' ummp=',ummp
!***
            CALL STARBURST_RADIATION_FIELD(AGE,UMMP,ISRF_WL(I),ISRF(I))

            WRITE(RADDESCRIPT,FMT='(A19,1PE10.2)')
     &         'starburst, age(yr)=',AGE

         ELSEIF(RADIATION.EQ.'BC_2003')THEN
!*** diagnostic
            if(i.eq.1)write(0,fmt='(a,1pe10.3,a,e10.3)')
     &         'vsg_stat_therm_v12 ckpt 9.52, age=',age,' ummp=',ummp
!***
            CALL BRUZUAL_CHARLOT_RF(AGE,ZBC,UMMP,ISRF_WL(I),ISRF(I))

            WRITE(RADDESCRIPT,FMT='(A7,0PF6.4,A7,1PE10.2)')
     &         'BC03 Z=',ZBC,' t(yr)=',AGE

         ELSEIF(RADIATION.EQ.'BPASS')THEN
!*** diagnostic
            if(i.eq.1)write(0,fmt='(a,1pe10.3,a,e10.3)')
     &         'vsg_stat_therm_v12 ckpt 9.53, age=',age,' ummp=',ummp
!***
            CALL BPASS_RF(AGE,ZBPASS,UMMP,ISRF_WL(I),ISRF(I))

            WRITE(RADDESCRIPT,FMT='(A7,0PF6.4,A7,1PE10.2)')
     &         'BPASS Z=',ZBPASS,' t(yr)=',AGE

         ELSEIF(RADIATION.EQ.'KURUCZ')THEN
!*** diagnostic
            if(i.eq.1)write(0,fmt='(a,1pe10.3,a,e10.3)')
     &         'vsg_stat_therm_v12 ckpt 9.54, Teff=',TEFF,
     &         ' ummp=',ummp
!***
            CALL KURUCZ_RF(TEFF,UMMP,ISRF_WL(I),ISRF(I))
!*** diagnostic
!            write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.55'
!***
            WRITE(RADDESCRIPT,FMT='(A,1PE10.3,A)')
     &         'Kurucz, Teff=',Teff,' K'
!*** diagnostic
!            write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 9.56'
!***
         ENDIF
...
! if GEOM=LIRG, obtain optically-thick radiation field

      IF(GEOM.EQ.'LIRG')THEN
         DO I=1,NISRF
            IF(AWAV(I).GT.0.01)THEN   ! AWAV = A_lambda/A_V
               ISRF(I)=ISRF(I)/AWAV(I)
            ELSE
               ISRF(I)=100.*ISRF(I)
            ENDIF
         ENDDO
...
! write out radiation field to file isrf.out
     ...
     &   '   (um)    (erg cm-3)'
      DO IWAV=1,NISRF
         WRITE(14,FMT='(1PE10.3,1PE10.3)')(1.E4*ISRF_WL(IWAV)),
     &      (ISRF_WL(IWAV)*ISRF(IWAV)/2.997925E10)
      ENDDO
      CLOSE(14)
...
! vsg_stat_therm.iout is output file with emission spectrum
     ...
     &      'P = time-averaged radiated power/grain'
      ELSEIF(GEOM.EQ.'SLAB')THEN
         WRITE(66,FMT='(A,/,A,A,/,A,A,F5.2,A)')
     &      'Starlight incident normally on slab,',
     &      'aeff=0.1um Astrodust (P=0.2, b/a=0.5) grain at front',
     &      ' surface absorbs power h=U*1.827e-12 erg/s',
     &      'P_j = probability of being in energy bin j',
     &      ' in slab of thickness A_V=',AVMAX,' mag'
         WRITE(77,FMT='(A,/,A,A,/,A,A,F5.2,A)')
     &      'Starlight incident normally on slab,',
     &      'aeff=0.1um Astrodust (P=0.2, b/a=0.5) grain at front',
     &      ' surface absorbs power h=U*1.827e-12 erg/s',
     &      'P = time-averaged radiated power/grain',
     &      ' in slab of thickness A_V=',AVMAX,' mag'
      ELSEIF(GEOM.EQ.'LIRG')THEN
         WRITE(66,FMT='(A,/,A,A,/,A)')
     &      'diffuse reddened starlight for mixed stars and dust',
     &      ' 0.1um Astrodust grain (P=0.2, b/a=0.5) absorbs power',
     &      ' h = U*1.829e-12 erg/s',
     &      'P_j = probability of being in energy bin j'
         WRITE(77,FMT='(A,/,A,A,/,A)')
     ...
     &      ' h = U*1.829e-12 erg/s',
     &      'P = time-averaged radiated power/grain'
         WRITE(66,FMT='(A)')'LIRG mode not implemented yet: halt'
         WRITE(77,FMT='(A)')'LIRG mode not implemented yet: halt'
         WRITE(0,FMT='(A)')'LIRG mode not implemented yet: halt'
         STOP
      ENDIF
...
! loop over grain size -----------------------------------------------------
 ...
 9940    format(80('='),/,'     a =',1pe10.3,' cm')
     ...
     &      ' about to call vsg_td_emission'
         ...
         IF(GEOM.EQ.'ISRF'.OR.GEOM.EQ.'LIRG')THEN
...
! spatial integration for slab

! initialize: AV=0 and ISRF to ISRF0

            AV=0.
            DO IWAV=1,NISRF
               ISRF(IWAV)=ISRF0(IWAV)
            ENDDO
...
! initialize emission per grain to zero

            DO IWAV=1,NISRF0
               EMISSION(IWAV)=0.
            ENDDO

! if GEOM='SLAB', use uniform zone spacing

            DO JZONE=1,NZONE

!*** diagnostic
               write(0,fmt='(a,i3)')
     &            'vsg_stat_therm_v12 ckpt 13, jzone=',jzone
!***

               IF(GEOM.EQ.'SLAB')THEN
                  DAV=AVMAX/NZONE
               ELSE
                  WRITE(0,*)'vsg_stat_therm_v12 ckpt 13.1',
     &               ' fatal error: stop'
                  STOP
               ENDIF
...
!*** diagnostic
!               write(0,fmt='(a,f6.2)')
!     &            'vsg_stat_therm_v12 ckpt 14, DAV=',DAV
!***
               DO IWAV=1,NISRF

! 2020.06.30 (BTD) correction: add factor 0.921

                  DTAU=0.921*DAV*AWAV(IWAV)   ! AWAV = A_lambda/A_V

! compute ISRFZ = effective radiation field within zone

                  IF(DTAU.GT.1.D-6)THEN
                     ISRFZ(IWAV)=ISRF(IWAV)*(1.-EXP(-DTAU))/DTAU
                  ELSE
                     ISRFZ(IWAV)=ISRF(IWAV)*(1.-0.5*DTAU)
                  ENDIF

! input for next zone:

                  ISRF(IWAV)=ISRF(IWAV)*EXP(-DTAU)

               ENDDO
               AV=AV+DAV

! try initializing PSTATEZ to zero
               DO JS=1,NSTATEMX
                  PSTATEZ(JS)=0.D0
               ENDDO
!----------------------------------
     ...
     &                              CASEDESCRIPT,RADDESCRIPT,
     &                              DTOH,A(ISIZE),PORO,
     &                              NISRF,ISRF_WL,ISRFZ,DLGLAMBDA,
     &                              CABS,CSCA,NSTATE,CDESCR,
     &                              UZ,UAZ,UBZ,LNGU,TZ,DELTA_T,PSTATEZ,
     &                              RATE_HEATING,COOLING_TIME,RAD_TIME,
     &                              AMATRIX,NSTATEZ1,PSTATE1,RHS,
     &                              AMATRIX1,NMAX,IJA,SA,
     &                              EMISSIONZ,ABSORPTION,PCUMULT,BGD,
     &                              NDIME,NDIMMODES,
     &                              E,EMODES,LNG,LNGSUM,SCRW)

!*** diagnostic
!               write(0,*)'vsg_stat_therm_v12 ckpt 16'
!***

! augment emission per grain

               DO IWAV=1,NISRF0
                  EMISSION(IWAV)=EMISSION(IWAV)+EMISSIONZ(IWAV)*DAV
               ENDDO

               IF(JZONE.EQ.1)THEN

                  NSTATE1=NSTATEZ1
                  IF(NSTATE1.EQ.1)THEN ! delta-function dist in zone 1
!*** diagnostic
                     write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 16.1'
!***
                     U(1)=UZ(1)
                     T(1)=TZ(1)
                     UA(1)=UZ(1)   ! place-holder: not meaningful
                     UB(1)=UZ(1)   ! place-holder: not meaningful
                     PSTATE(1)=DAV
                  ELSE                 ! full distribution in zone 1
                     DO JS=1,NSTATE1
                        U(JS)=UZ(JS)
                        UA(JS)=UAZ(JS)
                        UB(JS)=UBZ(JS)
                        T(JS)=TZ(JS)
                        PSTATE(JS)=PSTATEZ(JS)*DAV
                     ENDDO
                  ENDIF   ! endif nstate=1 ... else...

               ELSE   ! jzone > 1

                  IF(NSTATE1.EQ.NSTATE-1)THEN

! using full distribution: 
! check whether energy grid needs to be extended to lower energies

!*** diagnostic
                     write(0,fmt='(a)')'vsg_stat_therm_v12 ckpt 17'
!                     write(0,fmt='(a,1pe10.3,a,e10.3,a,e10.3)')
!     &                  '   ua(2)=',ua(2),' u(2)=',u(2),' ub(2)=',ub(2)
!                     write(0,fmt='(a,1pe10.3,a,e10.3,a,e10.3)')
!     &                  '   ua(1)=',ua(1),' u(1)=',u(1),' ub(1)=',ub(1)
!                     write(0,fmt='(a,1pe10.3)')'   uz(1)=',uz(1)
!                     write(0,fmt='(a,1pe10.3,a,e10.3)')
!     &                  '   uaz(1)=',uaz(1),' ubz(1)=',ubz(1)
!***
                     JNEW=0
                     DO JSZ=1,NSTATE-1
                        IF(UZ(JSZ).LT.UA(1))THEN
                           JNEW=JNEW+1
                        ELSE
                           GOTO 1100
                        ENDIF
...
!*** diagnostic
!                     write(0,fmt='(a,i3)')
!     &                  'vsg_stat_therm_v12 ckpt 18 JNEW=',jnew
!*** 

! if JNEW > 0, create JNEW new energy bins at low end
                     IF(JNEW.GT.0)THEN

! need to create new bin at low energies
! free bins NMID, NMID+1, ... , NMID+JNEW-1

! to free space, merge bins NMID and NMID+1        -> NMID+JNEW
!                         NMID+2 and NMID+3        -> NMID+JNEW+1
!                         ...
!                  NMID+2*JNEW-2 and NMID+2*JNEW-1 -> NMID+2*JNEW-1

                        NMID=NSTATE/2
                        DO JJ=JNEW,1,-1
                           J1=NMID+2*JJ-2
                           J2=J1+1
                           PSUM=PSTATE(J1)+PSTATE(J2)
                           U(NMID+JNEW+JJ-1)=(PSTATE(J1)*U(J1)+
     &                                        PSTATE(J2)*U(J2))/PSUM
                           T(NMID+JNEW+JJ-1)=(PSTATE(J1)*T(J1)+
     &                                        PSTATE(J2)*T(J2))/PSUM
                           PSTATE(NMID+JNEW+JJ-1)=PSUM
                           UB(NMID+JNEW+JJ-1)=UB(J2)
                           UA(NMID+JNEW+JJ-1)=UA(J1)
                        ENDDO

! free bins 1-JNEW
! move data upward by shift JNEW to bins JNEW+1 ... NMID+JNEW-1
! 
                        DO JS=NMID+JNEW-1,JNEW+1,-1
                           PSTATE(JS)=PSTATE(JS-JNEW)
                           UA(JS)=UA(JS-JNEW)
                           U(JS)=U(JS-JNEW)
                           UB(JS)=UB(JS-JNEW)
                           T(JS)=T(JS-JNEW)
...
! update PSTATE
                     ...
                     J1=1
                     JS=1
                     IF(UZ(1).EQ.0.D0)THEN
                        PSTATE(1)=PSTATE(1)+DAV*PSTATEZ(1)
!*** diagnostic
!                        write(0,*)'vsg_stat_therm_v12 ckpt 18.5',
!     &                            ' update pstate(1)'
!***
                        J1=2
                        JS=2
 ...
 2100                   CONTINUE
!*** diagnostic
!                        write(0,fmt='(a,a,i3,a,i3)')
!     &                     'vsg_stat_therm_v12 ckpt 18.6',
!     &                     ' update pstate(',js,') with jsz=',jsz
!***
                        IF(UZ(JSZ).GE.UA(JS).AND.UZ(JSZ).LT.UB(JS))THEN
                           PSTATE(JS)=PSTATE(JS)+PSTATEZ(JSZ)*DAV
                        ELSE
                           JS=JS+1

! it is possible that the solution for this zone is using an
! energy grid that extends beyond that used by zone 1.
! if so, population from such high energy bins will be
! allocated to js=nstate-1

                           IF(JS.GT.NSTATE1)THEN
                              JS=NSTATE1
!*** diagnostic
!                              write(0,fmt='(a,a,i3,a,i3)')
!     &                           'vsg_stat_therm_v12 ckpt 18.7:',
!     &                           ' allocate jsz=',jsz,' to js=',js
!***
                              PSTATE(JS)=PSTATE(JS)+PSTATEZ(JSZ)*DAV
                              GOTO 2200
                           ENDIF
                        ...
                        ENDIF
 2200                   CONTINUE
                     ENDDO   ! enddo jsz=j1,nstate1

! completed update for case where we have full energy distribution

                  ELSE

!                    using delta functions for all zones
!                    therefore need to add one more


                     NSTATE1=NSTATE1+1
!*** diagnostic
                     write(0,fmt='(a,i3)')
     &                  'vsg_stat_therm_v12 ckpt 19, nstate1=',nstate1
!***
                     DO JS=NSTATE1,2,-1
!*** diagnostic
!                        write(0,fmt='(a,i3,a,1pe10.3)')
!     &                     'vsg_stat_therm_v12 ckpt 19.1, js=',js,
!     &                     ' set pstate(js)=pstate(js-1)=',pstate(js-1)
!***
                        PSTATE(JS)=PSTATE(JS-1)
                        UB(JS)=UB(JS-1)
                        U(JS)=U(JS-1)
                        UA(JS)=U(JS)-(UB(JS)-U(JS))
                        T(JS)=T(JS-1)
                     ENDDO
!*** diagnostic
                     write(0,fmt='(a,1pe10.3)')
     &                   'vsg_stat_therm_v12 ckpt 19.5, dav=',dav
!***
                     PSTATE(1)=DAV
                     UA(1)=UAZ(1)
                     UB(1)=UBZ(1)
                  ...
                  ENDIF  ! endif(nstate1=nstate-1)
               ENDIF   ! endif(jzone.eq.1)

!*** diagnostic
!               do js=1,nstate1
!                  write(0,fmt='(a,i3,a,1pe10.3)')
!     &               'vsg_stat_therm_v12 ckpt 19.6 pstate(',js,
!     &               ')=',pstate(js)
!               enddo
!***
            ENDDO   ! enddo jzone=1,nzone

! completed summing over zones
! now normalize PSTATE

            DO JS=1,NSTATE1
               PSTATE(JS)=PSTATE(JS)/AV
            ENDDO
!*** diagnostic
!               do js=1,nstate1
!                  write(0,fmt='(a,i3,a,1pe10.3)')
!     &               'vsg_stat_therm_v12 ckpt 20 pstate(',js,
!     &               ')=',pstate(js)
!               enddo
!***

! normalize emission to average emission per grain in slab of thickness AV

            WRITE(77,FMT='(9(A,/),A,/,A)')
     &         (CDESCR(I),I=1,9),
     &         '   lambda   nu*P_nu     C_abs',
     &         '   (um)    (erg s-1)    (cm2)'

            DO IWAV=1,NISRF0
               EMISSION(IWAV)=EMISSION(IWAV)/AV
               WRITE(77,FMT='(F9.4,2(1PD12.4))')
     &            ISRF_WL(IWAV)*1.D4,EMISSION(IWAV),CABS(IWAV)
            ENDDO

!!!! add module to write out dP/dT (and emission?)

         ENDIF  ! endif geom=ISRF or geom=LIRG
